...
# """
def rename_cols(df: pd.DataFrame, prc_col: str = 'adj_close') -> None:
    df.columns = [str(x).lower().strip().replace('-', '_').replace(' ', '_') for x in df.columns]
...
def read_dat(pth, prc_col: str = 'adj_close') -> pd.DataFrame:
    return pd.DataFrame()
